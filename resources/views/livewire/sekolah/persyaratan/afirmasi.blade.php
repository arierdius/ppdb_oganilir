<div>
    <div class="container-fluid">
        <div class="page-header">
            <div class="row">
                <div class="col-sm-6">
                    <h3>Persyaratan Afirmasi</h3>
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item"><a href="index.html">Home</a></li>
                        <li class="breadcrumb-item">Persyaratan Afirmasi</li>
                    </ol>
                </div>
            </div>
        </div>
                        ...
                        <form class="row g-3 needs-validation"wire:submit.prevent="update">
                            <div class="col-md-12">
                                <table class="table table-bordered">
                                    ...
                                    <tbody>
                                        @forelse ($persyaratan as $key_afirmasi)
                                            @php
                                                $nama_surat = $key_afirmasi->nama_surat;
                                                $new_nama_surat = strtr($nama_surat, '_', ' ');
                                            @endphp
                                            <tr>
                                                <td>{{ $persyaratan->firstItem() + $loop->index }}</td>
                                                <td>{{ ucwords($new_nama_surat) }}</td>
                                                <td>{{ $key_afirmasi->nama_surat }}</td>
                                                <td>
                                                    <button wire:click.prevent="edit({{ $key_afirmasi->id }})" type="button" data-bs-toggle="modal" data-original-title="test"
                                                    data-bs-target="#editExample" class="btn btn-primary btn-xs">Edit</button>
                                                    <button wire:click="$emit('triggerDelete',{{ $key_afirmasi->id }})" class="btn btn-danger btn-xs" type="button">Delete</button>
                                                </td>
                                            </tr>
                                        @empty
                                        @endforelse
    ...
    <div wire:ignore.self class="modal fade" id="editExample" tabindex="-1" role="dialog"
        ...
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Perbarui Data Persyaratan</h5>
                </div>
                <form class="row g-3 needs-validation">
                    <div class="modal-body">
                        <div class="col-md-12">
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        @this.on('triggerDelete', id => {
            Swal.fire({
                ...
                html: "Anda tidak dapat mengembalikan File ini!",
                icon: 'warning',
