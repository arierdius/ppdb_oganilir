            ...
            <div class="row">
                <div class="col-sm-6">
                    ...
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item">Persyaratan Prestasi</li>
                    </ol>
                </div>
            </div>
        </div>
                        ...
                        <form class="row g-3 needs-validation"wire:submit.prevent="update">
                            <div class="col-md-12">
                                <table class="table table-bordered">
                                    ...
                                    <tbody>
                                        @forelse ($persyaratan as $key_prestasi)
                                            @php
                                                $nama_surat = $key_prestasi->nama_surat;
                                                $new_nama_surat = strtr($nama_surat, '_', ' ');
                                            @endphp
                                            <tr>
                                                <td>{{ $persyaratan->firstItem() + $loop->index }}</td>
                                                <td>{{ ucwords($new_nama_surat) }}</td>
                                                <td>{{ $key_prestasi->nama_surat }}</td>
                                                <td>
                                                    <button wire:click.prevent="edit({{ $key_prestasi->id }})" type="button" data-bs-toggle="modal" data-original-title="test"
                                                    data-bs-target="#editExample" class="btn btn-primary btn-xs">Edit</button>
                                                    <button wire:click="$emit('triggerDelete',{{ $key_prestasi->id }})" class="btn btn-danger btn-xs" type="button">Delete</button>
                                                </td>
                                            </tr>
                                        @empty
                                        @endforelse
